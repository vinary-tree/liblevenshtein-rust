...
Accept when some position reaches $`i = \lvert W\rvert`$; that position's $`e`$ is the match distance. Because the update depends only on $`\chi`$, one fixed transition rule (independent of $`W`$) serves every query. This crate **simulates** the deterministic Levenshtein automaton's states — reduced sets of positions, kept minimal by subsumption — directly during the dictionary walk: this is Schulz & Mihov's *imitation* method [[1]](#references). The default query path memoizes only the state/class cells it actually reaches. Eligible short Standard queries use an exact-cost bit-packed lazy DFA; the remaining algorithms use the same demand-driven scheme over positional frontiers. The pseudocode above is the recurrence computed on each cache miss. (A separate eager `universal` automaton, and the bit-vector *universal* construction of Mitankin et al. [[2]](#references), remain alternatives rather than the default path.)
