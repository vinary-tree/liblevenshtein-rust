...
pub fn base() -> &'static RuleSetChar {
    ...
    RULESET.get_or_init(|| {
        ...
        let file = crate::phonetic::llev::parse_str(content).expect(
            "Invalid embedded swedish/base.llev - this indicates an internal invariant violation",
        );
        RuleSetChar::from_llev(&file).expect(
            "Failed to compile Swedish base rules - this indicates an internal invariant violation",
        )
    })
}
