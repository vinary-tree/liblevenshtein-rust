//! Public-surface integration and property tests for exact ERP trie search.
...
fn range_knn_empty_boundaries_and_collisions_are_exact() {
    ...
    let index = ErpTransducer::from_series(QuantizationConfig::for_u8(0.0, 10.0), config, &series);
    ...
    assert_eq!(
        as_map(&index.search_range(&[], 3.0)),
        as_map(&brute_range(&series, &[], config, 3.0))
    );
...
fn public_configuration_and_mutation_surface_is_complete() {
    let mut index: ErpTransducer<&str> =
        ErpTransducer::new(QuantizationConfig::for_u8(-5.0, 5.0), ErpConfig::new(1.0));
    assert_eq!(index.kernel().gap_value(), 1.0);
...
fn non_finite_samples_are_outside_the_exact_search_domain() {
    let mut index: ErpTransducer<usize> = ErpTransducer::new(
...
proptest! {
    ...
    fn exact_range_is_differential_monotone_and_deterministic(
        ...
        let config = ErpConfig::new(f64::from(raw_g));
