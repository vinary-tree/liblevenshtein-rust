...
exact remote branch tips. The root second-wave run passed every completed job;
its Miri interpreter remains active, while its DragonFlyBSD startup is a proven
hosted-infrastructure hang relative to the successful BSD peers. A local
bounded-runner correction prevents either gate from consuming GitHub Actions'
six-hour default and awaits separate approval to push. No RC.5 tag or registry
...
| remote validation | coordinated branches resolve to approved commits and protected workflows pass | third-wave lling-llang and libdictenstein commits read back exactly and pass hosted CI; six owners are green; the root bounded-runner repair awaits push approval |
...
The root bounded-runner and operator-storage correction is committed locally
and has not been pushed:

| Artifact owner | Local CI-repair commit | Corrective scope |
|---|---|---|
| `liblevenshtein-rust` | `bb54735866dca33373a0b58d131dcc11a8e3ed36` | evidence-based Miri and BSD startup ceilings plus repository-local npm readback storage |
