...
exact remote branch tips. The root maintained-toolchain correction is pushed;
its replacement run has 48 of 52 jobs green while Miri and sanitizers remain
active. DragonFlyBSD installs the required dports compiler successfully, but
the bare `g++` probe does not reach the package-owned absolute path; selection
of the same-named base compiler is the evidence-supported explanation. The
absolute package-owned compiler correction passes archive, workflow, and local
header validation and awaits separate approval to push. No RC.5 tag or
registry coordinate has been published.
...
| remote validation | coordinated branches resolve to approved commits and protected workflows pass | third-wave lling-llang and libdictenstein commits pass hosted CI; root run 33118997183 has 48 of 52 jobs green, a locally repaired DragonFlyBSD PATH-resolution failure, one bounded transient NetBSD VM-start failure, and two active long-running jobs |
...
- 2026-08-27: exact approved root tip
  ...
  active.
- 2026-08-27: exact approved root tip
  `875e99a5bb94821afa958a811f957bf3541f7175` was pushed and read back.
  [Run 33118997183](https://github.com/vinary-tree/liblevenshtein-rust/actions/runs/33118997183)
  installed both `rust` and the `gcc` meta-package successfully, but its bare
  `g++` probe retained the older compiler's missing `<span>` behavior. Direct
  inspection of the official 820-byte `gcc-11_5.pkg` archive proves that the
  maintained package owns `/usr/local/bin/g++` as a symlink to `g++11`. The
  combined observations strongly imply that bare-name resolution selected the
  same-named base compiler before that package path; this is an inference from
  the hosted behavior and archive layout. Local
  commit `17f6184e77051aabf1e5a149362f682448a87533` therefore selects
  absolute compiler paths for both below-MSRV BSD systems, verifies
  executability, prints the selected compiler version, and emits failure-only
  feature-probe diagnostics. The exact script passes the complete header with
  GCC 13, its workflow YAML parses, and `git diff --check` passes. The same run
  had one independent NetBSD VM start time out after repeated SSH resets; both
  preceding exact-source runs passed that job, establishing a bounded hosted
  VM transient rather than a source regression.
...
The newly exposed DragonFlyBSD standard-library correction was first committed
locally before its approved push:
...
| Artifact owner | Remote branch tip | Corrective scope |
|---|---|---|
| `liblevenshtein-rust` | `875e99a5bb94821afa958a811f957bf3541f7175` | maintained DragonFly toolchain installation, library-capability probing, and exact hosted evidence |

The package-path correction is committed locally and has not been pushed:

| Artifact owner | Local CI-repair commit | Corrective scope |
|---|---|---|
| `liblevenshtein-rust` | `17f6184e77051aabf1e5a149362f682448a87533` | absolute package-owned BSD compiler selection and failure-only capability diagnostics |
