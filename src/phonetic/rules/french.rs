...
pub fn base() -> &'static RuleSetChar {
    ...
    RULESET.get_or_init(|| {
        ...
        let file = crate::phonetic::llev::parse_str(content).expect(
            "Invalid embedded french/base.llev - this indicates an internal invariant violation",
        );
        RuleSetChar::from_llev(&file).expect(
            "Failed to compile French base rules - this indicates an internal invariant violation",
        )
    })
}
...
mod tests {
    ...
    fn test_oi_digraph() {
        let rules = base();
        // moi → mwa (oi -> wa in IPA, then w -> v gives mva)
