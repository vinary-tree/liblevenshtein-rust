...
fn test_lexer_quantifier() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('a'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::QuantifierStart);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Number(2));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Comma);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Number(5));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::QuantifierEnd);
}
...
fn test_lexer_dot() {
    let mut lexer = Lexer::new("a.b");
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('a'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Dot);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('b'));
}
...
fn test_lexer_dollar_followed_by_space_is_anchor() {
    ...
    let mut lexer = Lexer::new("$ ");
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::EndOfLine);
}
...
fn test_lexer_byte_simple() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'a'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'b'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'c'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Eof);
}
...
fn test_lexer_byte_operators() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'a'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Star);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'b'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Plus);
}

#[test]
fn test_lexer_byte_arrow() {
    let mut lexer = LexerByte::new(b"ph -> f");
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'p'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'h'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Arrow);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), TokenByte::Byte(b'f'));
}
...
fn test_lexer_phonetic_shortcut_vowel() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "vowel", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "vowel", negated: true }
    );
}
...
fn test_lexer_phonetic_shortcut_consonant() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "consonant", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "consonant", negated: true }
    );
}
...
fn test_lexer_phonetic_shortcut_stop() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "stop", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "stop", negated: true }
    );
}
...
fn test_lexer_phonetic_shortcut_voiced() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "voiced", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "voiced", negated: true }
    );
}
...
fn test_lexer_phonetic_shortcut_fricative() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "fricative", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "fricative", negated: true }
    );
}
...
fn test_lexer_phonetic_shortcut_affricate() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "affricate", negated: false }
    );

    // \A outside char class is StartOfInput anchor (not negated affricate)
    let mut lexer2 = Lexer::new(r"\A");
    assert_eq!(lexer2.next_token().expect("test: lexer2.next_token must be Ok"), Token::StartOfInput);

    // Inside char class: both \a and \A are phonetic shortcuts
    let mut lexer3 = Lexer::new(r"[\a\A]");
    assert_eq!(lexer3.next_token().expect("test: lexer3.next_token must be Ok"), Token::CharClassStart);
    assert_eq!(
        lexer3.next_token().expect("test: lexer3.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "affricate", negated: false }
    );
    assert_eq!(
        lexer3.next_token().expect("test: lexer3.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "affricate", negated: true }
    );
}
...
fn test_lexer_standard_shortcut_digit() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "digit", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "digit", negated: true }
    );
}
...
fn test_lexer_standard_shortcut_word() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "word", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "word", negated: true }
    );
}
...
fn test_lexer_standard_shortcut_space() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "space", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "space", negated: true }
    );
}
...
fn test_lexer_shortcut_all_vowel_types() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "front_vowel", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "back_vowel", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "high_vowel", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "low_vowel", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "mid_vowel", negated: false }
    );
}
...
fn test_lexer_shortcut_consonant_types() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "glide", negated: false }
    );
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "liquid", negated: false }
    );

    // \z outside char class is EndOfInputStrict anchor
    let mut lexer2 = Lexer::new(r"\z");
    assert_eq!(lexer2.next_token().expect("test: lexer2.next_token must be Ok"), Token::EndOfInputStrict);
    ...
    assert_eq!(
        lexer3.next_token().expect("test: lexer3.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "nasal", negated: false }
    );
}
...
fn test_lexer_non_capturing_group() {
    let mut lexer = Lexer::new("(?:abc)");
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::NonCapturingGroupStart);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('a'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('b'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('c'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::GroupEnd);
...
fn test_lexer_inline_flags_case_insensitive() {
    ...
    let token = lexer.next_token().expect("test: lexer.next_token must be Ok");
    match token {
...
fn test_lexer_anchor_start_of_line() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::StartOfLine);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('h'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('e'));
...
fn test_lexer_anchor_start_of_input() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::StartOfInput);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('h'));
}
...
fn test_lexer_anchor_end_of_input() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('l'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('o'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::EndOfInput);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Eof);
}
...
fn test_lexer_anchor_end_of_input_strict() {
    ...
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('l'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Char('o'));
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::EndOfInputStrict);
    assert_eq!(lexer.next_token().expect("test: lexer.next_token must be Ok"), Token::Eof);
}
...
fn test_lexer_anchor_escapes_in_char_class() {
    ...
    assert_eq!(
        lexer.next_token().expect("test: lexer.next_token must be Ok"),
        Token::PhoneticShortcut { class_name: "nasal", negated: true }
    );
...
fn test_lexer_dollar_symbol_vs_anchor() {
    ...
    assert_eq!(lexer2.next_token().expect("test: lexer2.next_token must be Ok"), Token::EndOfLine);

    let mut lexer3 = Lexer::new("$ ");
    assert_eq!(lexer3.next_token().expect("test: lexer3.next_token must be Ok"), Token::EndOfLine);
}
