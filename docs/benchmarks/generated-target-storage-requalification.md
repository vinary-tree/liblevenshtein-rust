...
The repository manifest gives `libdictenstein` and the optional
`vinary-tree-interop` ABI crate exact version requirements together with sibling
paths for the coordinated development graph. Release CI checks out those
siblings at the exact source references recorded by the release manifest before
it packages this crate. Cargo then normalizes each path-plus-version dependency
to its registry coordinate in the published package. The resulting crate is
standalone for consumers, while a source checkout retains reproducible access
to the not-yet-published release-candidate graph without depending on whichever
versions happen to be newest in a registry.
