...
| `term_index` (`i`) | characters consumed from the query term; invariant `0 ≤ i ≤ m` |
...
conversion helpers (`usize::MAX`, `i64::MAX`, `±∞`, `NaN`, huge-finite inputs).
...
The naive unit-cost bound `|i − j| ≤ f − e` is only correct when `max(insertion, deletion) ≤ 1`;
...
The fix charges `(m − i) · deletion` **and** restricts acceptance to positions with `i ≤ m`
...
`ε`-tolerant `msm_subsumes` pruning relation, which is unchanged.
...
alphabet, lengths `0..=5`, `insertion = deletion ∈ {1, 1.5, 2, 3}`, random substitution), plus the
...
- **`OperationCostsF64::is_valid`** now also requires finiteness (`+∞` previously passed and would
...
- **Invariant T1 — every pruning lower bound is admissible.** The length lower bound `|m − n| · c`
...
- **Invariant T2 — tie ordering is deterministic.** `search_empty_query` (`τ = +∞`),
...
- **Invariant T3 — quantization is total.** `quantize`/`quantize_u8`/`quantize_u16` clamp
  `NaN`/`±∞`/out-of-range to valid bins; SAX `zscore_to_symbol` maps `NaN → 0` for parity.
