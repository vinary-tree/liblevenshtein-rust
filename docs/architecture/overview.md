...
|---|---|---|
| **liblevenshtein** (this crate, `v4.0.0-rc.1`) | — | the Levenshtein **transducer/automata**, edit-distance functions, pre-filters, higher-level reusable engines (phonetic, time-series MSM, WallBreaker, contextual completion, fuzzy cache), plus WASM/FFI/serialization surfaces |
...
| **libdictenstein** (`v4.0.0-rc.1`) | **exact package dependency with a development path** (`Cargo.toml`: `path` plus `version = "=4.0.0-rc.1"`) | **all dictionary backends** (`DoubleArrayTrie`, `DynamicDawg`/`DynamicDawgU64`, `SuffixAutomaton`, `Scdawg`, `PersistentARTrie`, `PathMapDictionary`) and the `Dictionary` / `DictionaryNode` / `MappedDictionary` traits, plus SIMD + bloom-filter pruning and prefix zippers |
