...
recycling arena and consumes each slot once. Query schedulers therefore do not
duplicate backend ownership logic.
...
| Snapshot-scoped cursor traversal | Direct for every scheduler whose dictionary walk does not expose child handles; adapted for schedulers that must materialize an accepting node | Generic `TraversalSession<N>` plus session-discriminated `TraversalCursor<N::SnapshotCursor>`; `DeferredNodeSource<N>` delays accepting-node materialization | One retained revision owns every cursor; graph handles drop before that owner; the session mode proves the active union member; native pointer cursors preserve provenance; owned-fallback slots are consumed or explicitly discarded exactly once |
...
| Lazy bounded DFS edge pages | Direct for prefix DFS over compact graphs and native backends advertising efficient cursor paging; rejected for exhaustive subsequence DFS | Generic `DfsNodeEdges<N>` selects an eager compatibility source or a `SmallVec`-backed cursor page; byte and char DATs expose the paging capability and transparent decorators forward it | Finality is captured before the first page; edge order and total count remain stable for the snapshot; one page holds at most eight descriptors without allocation; owned and non-paging backends remain eager; subsequence retains eager expansion because it has no automaton-dead edge pruning |
