...
exact remote branch tips. The root bounded-runner correction is pushed and its
replacement run has 49 of 52 jobs green; Miri and sanitizers remain active.
DragonFlyBSD no longer hangs, but its older compiler exposed the historical
`c++2a` spelling of its C++20 mode. The capability-probed dialect correction
passes both local header variants and awaits separate approval to push. No RC.5
...
| remote validation | coordinated branches resolve to approved commits and protected workflows pass | third-wave lling-llang and libdictenstein commits pass hosted CI; root replacement run 33115548329 has 49 of 52 jobs green; its locally repaired DragonFlyBSD dialect failure and two active long-running jobs remain |
...
The DragonFlyBSD C++20 dialect correction is committed locally and has not
been pushed:
