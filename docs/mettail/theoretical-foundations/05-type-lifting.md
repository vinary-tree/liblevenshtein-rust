...
2. **T_oper**: Operation declarations of form `Γ ⊢ f(x⃗) : B`
3. **T_prop**: Proposition declarations (typically `p : Pr, q : Pr ⊢ p ⇝ q` for rewriting)
...
| `fn sym f: A -> B`       | `T_oper: a : A ⊢ f(a) : B`       | Operation / function symbol        |
| `rewrite r: A -> R`      | `T_prop: p,q : Pr ⊢ p ⇝ q`       | Rewrite proposition                |
| `equation e`             | `T_ent: Γ \| Φ ⊢ t ⇝ u`          | Entailment / inference rule        |
...
- Propositions are subobjects φ ↣ Γ
- Entailment corresponds to inclusion
- Substitution is pullback
- Subset types {x : A | φ} arise naturally
...
(Definition 1 in Wells & Stay's paper). A **meta-context** `⟨−⟩ : T → T` is an
...
This says: "given contexts Γᵢ and rewrite hypotheses pᵢ ⇝ qᵢ, the operation f
...
An **encoding** `[[−]] : S → T` from source calculus S to target calculus T must satisfy:

1. **Preserve products**: `[[Γ × Δ]] = [[Γ]] × [[Δ]]`
2. **Preserve pullbacks**: `[[φ[σ]]] = [[φ]][[σ]]` (substitution commutes with encoding)
3. **Preserve abstraction**: `[[[A → B]]] = [[[A] → [[B]]]]`
4. **Map process type appropriately**: `Pr_S` maps to `⟨Pr_T⟩` for some meta-context ⟨−⟩
5. **Preserve rewriting**: `p ⇝ q ⊢ [[p]] →*_τ [[q]]` (source rewrites become silent paths)
6. **Preserve bisimilarity**: `p ≈ q ⊢ [[p]] ≈ [[q]]`
