    ...
    D: Dictionary,
{
    ...
    fn traversal_root(&self) -> DictionaryTraversalRoot<Self::Node> {
        ...
        map_transparent_traversal_root(self.inner.traversal_root(), |inner| {
            MemoryPressureNode::new(inner, metadata)
        })
    }
...
impl_transparent_dictionary_node!(MemoryPressureNode, |owner, child| MemoryPressureNode::new(
    child,
    Arc::clone(&owner.metadata),
));
impl_transparent_mapped_dictionary_node!(MemoryPressureNode);
