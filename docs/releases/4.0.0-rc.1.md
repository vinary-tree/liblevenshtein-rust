...
| `libdictenstein` | `6335a9d` | clean-source tag validation run `32705525805` succeeded | crates.io publication pending |
| `liblevenshtein-rust` | release branch includes the clean-source fix `d43cd83`; final tag awaits the commit containing this ledger | local default/all-feature suites passed; run `32739900519` proved the exact registry-dependency gate and stopped because libdictenstein was not public; final exact-tag validation pending | publication pending libdictenstein read-back |
| `lling-llang` | `2af235f` in isolated `release/4.0.0-rc.1` worktree | the earlier run `32705526551` succeeded at `4fffc88`; the shipped npm-guide correction invalidated that evidence, so complete exact-tag revalidation is pending | publication pending Rust prerequisites |
| `duallity` | `f0a644e` in isolated `release/4.0.0-rc.1` worktree | the earlier run `32705524747` succeeded at `60fb805`; the shipped binding-guide correction invalidated that evidence, so complete exact-tag revalidation is pending | publication pending Rust prerequisites |
...
| `libdictenstein@4.0.0-rc.1` on crates.io | pending | locked `cargo publish --dry-run` packaged 627 files, verified the crate, and reached the dry-run upload boundary |
| `liblevenshtein@4.0.0-rc.1` on crates.io | pending | blocked intentionally until libdictenstein resolves publicly |
