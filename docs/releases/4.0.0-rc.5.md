...
**State:** coordinated RC.5 branches and pull requests include the second
hosted-CI repair wave. Five owners pass their complete hosted workflows. The
root rerun has passed every completed job and awaits its long-running Miri and
DragonFlyBSD jobs. The lling-llang rerun passed its Rust, sanitizer, TLA+,
supply-chain, and binding gates but exposed one remaining Rocq 9.1 command-line
migration defect. That repair and a GHC 9.14 portability warning repair in
libdictenstein pass their local causal checks and await separate approval to
push. No RC.5 tag or registry coordinate has been published.
...
| remote validation | coordinated branches resolve to approved commits and protected workflows pass | second-wave root and lling-llang commits read back exactly; five owners pass hosted CI; the root rerun awaits two long-running jobs; locally validated third-wave lling-llang and libdictenstein repairs await push approval |
...
The third hosted-CI repairs are committed locally and have not been pushed:
