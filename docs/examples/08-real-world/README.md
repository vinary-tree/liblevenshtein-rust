...
>   range query touches `𝒪(k · log n)` nodes instead of all `n`.
...
1. a map from each **normalized form** back to the original term(s) for `𝒪(1)` exact
   lookups, and
...
A normalized index turns an otherwise `𝒪(∣D∣)` phonetic scan into an exact-map hit plus a
